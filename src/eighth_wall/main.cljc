(ns eighth-wall.main
  "Kotodama WASM entrypoint for the 8th_wall clean-room actor (L4) — Clojure port.
  ...
  NOTE: the actor handle is `8th_wall-compat` and the EAVT prefix is the
  string `8th_wall`, but a Clojure namespace symbol cannot begin with a
  digit, so this ns uses the valid alias `eighth-wall.main`. The handle and
  EAVT prefix (data, not symbols) keep the canonical `8th_wall` form.

  State lives on the kotoba Datom log: `emit-facts` produces namespaced EAVT
  facts (`8th_wall.<Entity>/<field>`); `*store*` is the in-memory
  materialization used by the contract test and by the WASM runtime before a
  live engine binds."
  (:require [clojure.string :as str]))

(def ns-prefix "8th_wall")
(def tier "L4")
...
(defn page-limit
  ...
  src/eighth_wall/page_limit.kotoba."
  [requested]
  (if (pos? requested)
...
(defn as-int-kernel
  ...
  Kotoba twin of as-int in paginate.kotoba; the string/parse half of
  eighth-wall.main/as-int stays here as the retained oracle."
  [v]
  (if (pos? v) v 0))
...
(defn emit-facts
  "EAVT facts for one record: {\"8th_wall.<Entity>/<field>\" v ...}. The datomic
...
(defn healthz [] [{:status "ok" :actor "8th_wall-compat" :tier tier :entities entities} 200])
...
(defn start! [] :8th_wall-compat/ready)
